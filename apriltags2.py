...
if __name__ == '__main__':
    ...
    at_detector = Detector(options, searchpath=['apriltags2'])
    ...
    detections = at_detector.detect(img, return_image=False)
    ...
    for image_name in image_names:
        ...
        start = time.time()
        detections = at_detector.detect(img, return_image=False)
        pose, e0, e1 = at_detector.detection_pose(detections[0],
                                          camera_params,
                                          parameters['rotation_test']['tag_size'])
        time_sum+=time.time()-start
        time_num+=1
        print(detections[0].tostring(
            collections.OrderedDict([('Pose',pose),
                                     ('InitError', e0),
                                     ('FinalError', e1)]),  indent=2))
        print(parameters['rotation_test']['posx'], parameters['rotation_test']['posy'], parameters['rotation_test']['posz'])
        print(parameters['rotation_test']['rotx'], parameters['rotation_test']['roty'], parameters['rotation_test']['rotz'])
    ...
    for image_name in image_names:
        ...
        poses = []
        start = time.time()
        tags = at_detector.detect(img, return_image=False)
        for detection in tags:
            pose, e0, e1 = at_detector.detection_pose(detection,
                                              camera_params,
                                              parameters['multiple_tags_test']['tag_size'])
        time_sum+=time.time()-start
        time_num+=1
